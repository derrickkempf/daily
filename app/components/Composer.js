...
export default function Composer({ onPosted }) {
  ...
  const [providers, setProviders] = useState([])     // [{id,label,credsOk,connected,approvalRequired}]
      ...
      {providers.length > 0 && (
        <>
          ...
          <div className="cross-post-grid">
            {providers.map(p => {
              const unavailable = !p.credsOk || !p.connected
              return (
                <label key={p.id}
                ...
                >
                  ...
                  />
                  {p.label}
                  <span className="cross-post-status">
                    {!p.credsOk ? 'no creds'
